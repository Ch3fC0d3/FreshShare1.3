  ...
  return new Promise((resolve) => {
    ...
    const req = client.request(SITE_URL, (res) => {
      ...
      res.on('end', () => {
        ...
        // Check CSS loading
        if (data.includes('<link rel="stylesheet" href="/css/color-system.css">')) {
        ...
        if (data.includes('503 Service Unavailable')) {
          console.log('[Remote] ❌ 503 Service Unavailable detected in page content');
        }
...
async function runChecks() {
  ...
  console.log('\n=== Check Complete ===');
  console.log('If you see any issues above, please check the GitHub Actions logs');
