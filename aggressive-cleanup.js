  ...
  function findNodeModules(dir) {
    try {
      ...
      for (const entry of entries) {
        ...
        if (entry.name === 'node_modules' && fullPath !== path.join(rootDir, 'node_modules')) {
          try {
            backupDirectory(fullPath);
            fs.rmSync(fullPath, { recursive: true, force: true });
            log(`Removed non-root node_modules: ${path.relative(rootDir, fullPath)}`, 'success');
            removedCount++;
          } catch (error) {
            ...
            try {
              ...
              if (stats.size > 500 * 1024) {
                if (backupFile(fullPath)) {
                  fs.unlinkSync(fullPath);
                  log(`Removed large image: ${path.relative(rootDir, fullPath)} (${(stats.size / 1024).toFixed(2)} KB)`, 'success');
                  removedCount++;
                }
              }
            } catch (error) {
...
function cleanup() {
  ...
  log(`Removed ${removedNodeModules} non-root node_modules directories`, 'info');
  ...
  log('3. Commit with message:                 git commit -m "Clean repository for production"', 'info');
  log('4. Push to deployment branch:           git push origin restore_branch', 'info');
