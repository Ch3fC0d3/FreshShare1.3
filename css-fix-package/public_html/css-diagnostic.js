...
document.addEventListener('DOMContentLoaded', function() {
  ...
  diagnosticArea.style.cssText = 'position:fixed; top:10px; right:10px; width:350px; padding:15px; background:#fff; border:2px solid #f00; z-index:9999; overflow:auto; max-height:80vh; font-family:monospace; font-size:12px;';
  ...
  closeBtn.onclick = function() { diagnosticArea.remove(); };
  ...
  cssLinks.forEach(link => {
    ...
    const url = href.startsWith('http') ? href : (href.startsWith('/') ? window.location.origin + href : window.location.origin + '/' + href);
    ...
    .then(response => {
      if (response.ok) {
        ...
        console.error('CSS failed to load:', url, response.status, response.statusText);
      }
    })
    .catch(error => {
      item.innerHTML = `<span style="color:red">❌</span> ${href}<br>
  ...
  Array.from(document.styleSheets).forEach((sheet, index) => {
    ...
    try {
      ...
      console.log(`Stylesheet #${index} has ${ruleCount} rules:`, sheet.href || 'inline');
    } catch (e) {
  ...
  fetch(testUrl)
    .then(response => {
      ...
      console.log('Headers for', testUrl, 'Content-Type:', response.headers.get('content-type'));
    })
    .catch(error => {
