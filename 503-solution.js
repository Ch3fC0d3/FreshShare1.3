...
app.use(
  ...
  createProxyMiddleware({
    ...
    onProxyReq: (proxyReq, req, res) => {
      console.log(
        `[PROXY] Forwarding request to backend: ${proxyReq.method} ${proxyReq.path}`
      );
    },
    onProxyRes: (proxyRes, req, res) => {
      console.log(
        `[PROXY] Received response from backend: ${proxyRes.statusCode}`
      );
    },
    onError: (err, req, res) => {
...
mockServer.listen(MOCK_PORT, () => {
  ...
  app.listen(PROXY_PORT, () => {
    ...
    console.log(
      `1. http://localhost:${PROXY_PORT}/test - Should return "Express proxy is working"`
    );
    console.log(
      `2. http://localhost:${PROXY_PORT}/api/pack/test - Should be proxied to backend`
    );
    console.log(
      `3. http://localhost:${MOCK_PORT}/test - Direct access to mock backend`
    );
...
function testProxyConnection() {
  ...
  http
    .get(`http://localhost:${PROXY_PORT}/api/pack/test`, (res) => {
      ...
      console.log(
        '2. Verify your proxy configuration in server.js points to http://localhost:8080'
      );
      console.log(
        '3. Use the config-temp.js files to set environment variables correctly'
      );
      console.log(
        '4. Start both servers in the correct order: first backend, then Express'
      );
    })
    .on('error', (err) => {
