...
// ---------- Utilities ----------
function assertNever(x) { throw new Error(`Unexpected: ${x}`); }
// GS1-128 minimal parser: (01)=GTIN-14, (37)=units per case, (310x)=net kg w/ x decimals
function parseGS1128(text) {
    function get(ai) {
        const marker = `(${ai})`;
...
function computeSuggestions(caseSize, shareSize, currentShares) {
    const safe = (n) => Number.isFinite(n) && n > 0;
    let warn = '';
    if (!safe(caseSize) || !safe(shareSize)) {
        return { warn: 'Enter a positive case size and share size to see suggestions.', K: 0, KDisplay: '—', neededToFill: '—', divisorOptions: [], suggestions: [] };
    }
    ...
    if (isInt) {
        ...
        if (need > 0)
            suggestions.push({ buy: need, completes: 1 });
        [1, 2, 3, 4, 5].forEach((m) => {
...
app.post('/parse-label', async (req, reply) => {
    const body = ParseLabelBody.parse(req.body);
    return parseGS1128(body.text);
});
app.get('/case-pack', async (req, reply) => {
    const gtin_case = req.query?.gtin_case;
    if (!gtin_case)
        return reply.code(400).send({ error: 'gtin_case required' });
    const { rows } = await pool.query(`SELECT cp.*, ti.gtin_each, ti.gtin_case, ti.category, ti.unit_net, ti.unit_uom
     FROM case_pack cp
     LEFT JOIN trade_item ti ON ti.gtin_case = $1 AND cp.trade_item_id = ti.id
     WHERE ti.gtin_case = $1
     ORDER BY cp.created_at DESC`, [gtin_case]);
    return { items: rows };
});
app.post('/case-pack', async (req, reply) => {
    const b = UpsertCasePackBody.parse(req.body);
    // upsert trade_item by gtin_case
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        const ti = await client.query(`INSERT INTO trade_item (gtin_each, gtin_case, category, unit_net, unit_uom)
       VALUES ($1,$2,$3,$4,$5)
       ...
       RETURNING id`, [b.gtin_each || null, b.gtin_case, b.category || null, b.unit_net || null, b.unit_uom || null]);
        const trade_item_id = ti.rows[0].id;
        const cp = await client.query(`INSERT INTO case_pack (trade_item_id, units_per_case, case_net_weight, case_uom, source, confidence, evidence_url)
       VALUES ($1,$2,$3,$4,$5,$6,$7)
       RETURNING *`, [trade_item_id, b.units_per_case || null, b.case_net_weight || null, b.case_uom || 'lb', b.source, b.confidence || 0.9, b.evidence_url || null]);
...
app.get('/resolve/case-weight', async (req, reply) => {
    ...
    if (q.gtin_case) {
        const { rows } = await pool.query(`SELECT cp.case_net_weight, cp.case_uom, cp.source
       FROM case_pack cp
    ...
    if (q.category) {
        const { rows } = await pool.query(`SELECT default_case_weight, uom FROM commodity_pack
...
app.post('/autosplit', async (req, reply) => {
    const b = AutoSplitBody.parse(req.body);
